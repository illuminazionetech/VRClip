...
`keystore.properties` exists at the repo root, so you don't need a real signing key
...
- `app/`: the application module.
  - `ui/`: Jetpack Compose screens (`ui/page/`), reusable components
    ...
    the motion tokens under `ui/common/motion/`).
...
- Run the commands above before opening a PR, since CI will run the same checks.
